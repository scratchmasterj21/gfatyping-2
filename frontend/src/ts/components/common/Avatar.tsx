...
export function Avatar(props: {
  ...
  celebrate?: boolean;
  /**
   * Literal pixel size. Only used to size the SVG viewport for the
   * procedural avatar (harmless to always pass, since a `class` with its
   * own height/width utility wins there - SVG width/height are
   * presentation attributes, the lowest-priority way to size something in
   * CSS).
   *
   * For the animal-image branch specifically, omit this prop entirely if
   * `class` already sets the size (e.g. the nav bar/leaderboard's
   * `h-[1.25em] w-[1.25em]`) - CSS layers in this app mean there's no
   * reliable specificity trick to make an inline/injected size rule lose to
   * a Tailwind utility class, so when `size` is omitted the image wrapper
   * just fills its parent (100%/100%) instead of setting its own size,
   * leaving sizing entirely to `class`.
   */
  size?: number;
  class?: string;
  /** An equipped animal avatar's image URL - when set, fully replaces the procedural avatar below with this flat image. */
  animalImage?: string;
}): JSXElement {
  ...
  return (
    ...
    >
      {(image) => (
        // Fixed-size, clipped wrapper (not just style on the <img> itself)
        // so the rendered box can never exceed the intended size regardless
        // of what CSS the <img> ends up subject to (Tailwind's preflight
        // sets `img { max-width: 100%; height: auto }`, which silently
        // overrides plain width/height attributes and let the image render
        // at its natural size, spilling into whatever sits next to it).
        //
        // When props.size is omitted, no width/height goes into the inline
        // style at all (not even 100%) - an inline style on this element
        // would always beat a class regardless of specificity/layers, so
        // leaving the size out entirely is what actually lets `class`
        // control it. See the `size` prop's doc comment for why this can't
        // just be a CSS specificity trick instead.
        <div
          class={cn("shrink-0 overflow-hidden", props.class)}
          style={{
        ...
        >
          <style>
            {`
          .avatar-img-bob { animation: avatar-img-bob 2.6s ease-in-out infinite; }
          @keyframes avatar-img-bob {
            0%, 100% { transform: translateY(0); }
            50% { transform: translateY(-2px); }
          }
          .avatar-img-cheer { animation: avatar-img-cheer 0.7s ease-in-out infinite; }
          @keyframes avatar-img-cheer {
            0%, 100% { transform: translateY(0) scale(1) rotate(0deg); }
            25% { transform: translateY(-6px) scale(1.06) rotate(-4deg); }
            50% { transform: translateY(0) scale(1.02) rotate(0deg); }
            75% { transform: translateY(-6px) scale(1.06) rotate(4deg); }
          ...
          @media (prefers-reduced-motion: reduce) {
            .avatar-img-bob, .avatar-img-cheer { animation: none; }
          }
        `}
          </style>
          <img
            src={image()}
            alt="avatar"
            class={props.celebrate ? "avatar-img-cheer" : "avatar-img-bob"}
            style={{
              width: "100%",
              height: "100%",
              "object-fit": "contain",
